...
void parse_strtab(char* buffer, long size)
{
  printf("[%s]\n", __func__);
  buffer++;
  size--;
  int npos = 1;
  while (size > 0) {
    auto n = printf("%s ", buffer);
    strtab[npos] = buffer;
    int k = strtab[npos].size() + 1;
...
bool process_file(void* _vPtr)
{
  ...
  for (int i=0; i<ehdr->e_shnum; i++) {
    ...
    case SHT_STRTAB: {
      parse_strtab(buffer+shdr->sh_offset, shdr->sh_size);
      break;
    }
    default:
      break;
...
int old_process_file(const char* filename)
{
